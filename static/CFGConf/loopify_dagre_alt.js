...
var loopify_dagre = (function(myExport){
  ...
  myExport.init = function(dotFile, filteredGraph, json){
    initVariables(dotFile, filteredGraph, json);
	  modifyDotGraph();
  ...
  function initVariables(dF, filteredGraph, json){  
		modifiedDotFile = "";
		currRootLoop = null;
    ...
    //TODO: This part only works on non-filtered graph currently
    // Make it work with filtered graph
    // For that, we need to make a function with reverse functionality
    // compared to getLoopifyTree
    // getLoopifyTree converts from objects in "loops" format as defined in
    // the schema to loopsObj as used by the loopify_dagre module.
    // This new function needs to convert from loopsObj format to "loops"
    // format
    if("loopAnalysis" in json){
      loopsObj = json["loopAnalysis"];
    } else {
